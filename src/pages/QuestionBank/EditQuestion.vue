...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchExamQuestions = (page = 1) => {
      ...
      api
        .get(
          "/questionbank-questions/?include=question&search=questionbank_id:" +
            route.params.id +
            "&searchJoin=and&limit=0"
        )
        .then((res) => {
          examQuestions.value = [];
  ...
  methods: {
    ...
    searchExams(searchData) {
      ...
      api
        .get(
          `/questionbank-questions?searchJoin=and&search=title:${searchData.keywords}&orderBy=id&sortedBy=desc&limit=0`
        )
        .then((res) => {
